...
const Sidebar = () => {
    const [activeIcon, setActiveIcon] = useState<string>(navItems[0].title)
  return (
        ...
        <div id="navItemsContainer" className="mt-12 hover:cursor-pointer">
            {
                navItems.map((x, index) => (
                    ...
                    id="navItem" 
                    key={index} 
                    className="flex items-center text-xl font-medium rounded-lg mb-6 h-16 hover:bg-[#141519]"
                    onClick={() => setActiveIcon(x.title)}>
                        <div id="navIcon" className={`bg-[#141519] p-3 rounded-[50%] my-0 mx-4 grid place-items-center ${x.title == activeIcon ? 'text-[#3773f5]' : ''}`}>
                            {x.icon}
                        </div>
                        <div>{x.title}</div>
                    </div>
